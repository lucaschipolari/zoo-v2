package Vista;

import Utilidades.Mensaje;
import javax.swing.JFrame;
import javax.swing.JOptionPane;

public abstract class VistaBase extends JFrame implements IVista {

    public VistaBase() {
        super();
    }
    
    @SuppressWarnings("unchecked")
    // <editor-fold defaultstate="collapsed" desc="Generated Code">//GEN-BEGIN:initComponents
    private void initComponents() {

        setDefaultCloseOperation(javax.swing.WindowConstants.EXIT_ON_CLOSE);

        javax.swing.GroupLayout layout = new javax.swing.GroupLayout(getContentPane());
        getContentPane().setLayout(layout);
        layout.setHorizontalGroup(
            layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
            .addGap(0, 400, Short.MAX_VALUE)
        );
        layout.setVerticalGroup(
            layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
            .addGap(0, 300, Short.MAX_VALUE)
        );

        pack();
    }// </editor-fold>//GEN-END:initComponents

    @Override
    public void MostrarComoDialogo() {
        pack();
        setLocationRelativeTo(null);
        setVisible(true);
    }

    @Override
    public void MostrarMensaje(String mensaje, Mensaje tipo) {
        JOptionPane.showMessageDialog(this, mensaje, tipo.toString(), EstablecerIcono(tipo));
    }

    @Override
    public boolean MostrarConfirmacion(String mensaje) {
        int resultado = JOptionPane.showConfirmDialog(this, mensaje,"",JOptionPane.YES_NO_OPTION);
        return resultado == JOptionPane.YES_OPTION;
    }
    
    private int EstablecerIcono(Mensaje tipo){
        switch(tipo){
            case EXITO:
                return JOptionPane.INFORMATION_MESSAGE;
            case ADVERTENCIA:
                return JOptionPane.WARNING_MESSAGE;
            default:
                return JOptionPane.ERROR_MESSAGE;
        }
    }

    // Variables declaration - do not modify//GEN-BEGIN:variables
    // End of variables declaration//GEN-END:variables
}
